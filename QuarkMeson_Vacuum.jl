...
@fastmath function quarkFlow(ρϕ, hϕ, k, Nc, Nf, d)
  mq = hϕ * ρϕ / Nf
  flowq = 1 / (k^2 + mq)
  return -Nc * Nf * 4 * Ω(d) / (2π)^d * k^(d + 2) / d * flowq
end

# all meson contributions to the flow of V_k(ρϕ)
@fastmath function mesonFlow(mπ2, mσ2, k, Nf, d)
  flowπ = 1 / (k^2 + mπ2)
  flowσ = 1 / (k^2 + mσ2)
  return Ω(d) / (2π)^d * k^(d + 2) / d * (flowσ + (Nf^2 - 1) * flowπ)
end
...
@fastmath @inbounds function kernel!(du, u, p::ParametersQM, t)
  ...
  # evaluate the flows
  try
    @.. thread = true flux .= -(quarkFlow.(ρϕ_grid, p.hϕ, k, p.Nc, p.Nf, p.d)
                                .+
                                mesonFlow.(u, Ms2, k, p.Nf, p.d)
    )
  catch e
...
parameters = ParametersQM(3, grid, 1, 2, 3, 0.65, 71.6, 0.0, 7.2);
# show the parameters
dump(parameters)
